...
def load_graph_edges(hashtag, window_date, base_path, graph_type='audience_projection'):
    """
    Load graph edges based on graph_type from config.
    
    Args:
        graph_type: 'audience_projection' or 'bipartite'
    """
    windows_dir = Path(base_path.format(hashtag=hashtag))
    
    if graph_type == 'audience_projection':
        # Audience-projected graph (s2_proj/count_B_imported)
        edges_path = windows_dir / window_date / 's2_proj' / 'count_B_imported' / 'edges.csv'
    elif graph_type == 'bipartite':
        # Original bipartite graph
        edges_path = windows_dir / window_date / 's1_bipartite' / 'edges.csv'
    else:
        raise ValueError(f"Unknown graph_type: {graph_type}. Use 'audience_projection' or 'bipartite'")
...
def pool_embeddings(embeddings, graph_info):
    ...
    features = {
        ...
        'graph_type': graph_info.get('graph_type', 'unknown'),  # NEW: store graph type
    }
...
def process_hashtag(hashtag, config, sample_threshold=7000, prefix='emb'):
    """
    Process one hashtag and extract embedding features.
    
    Args:
        prefix: Prefix for feature names (e.g., 'emb_aud', 'emb_bip')
    """
    base_path = config['input']['network_windows']
    
    # Get which graph types to use from config
    graph_types = config['embeddings'].get('graphs', ['audience_projection'])
    
    # For now, use the first graph type (can extend to process multiple later)
    graph_type = graph_types[0] if graph_types else 'audience_projection'
    ...
    for _, window_date in sorted(windows):
        edges = load_graph_edges(hashtag, window_date, base_path, graph_type=graph_type)
        ...
        # Use custom prefix instead of hardcoded 'emb'
        ...
        if prev_features and config['embeddings']['pooling'].get('compute_delta', False):
            for key, val in features.items():
                if key in prev_features and not key.startswith('graph_'):
                    row[f'{prefix}_{key}_delta'] = val - prev_features[key]
...
def main():
    ...
    parser.add_argument('--prefix', type=str, default='emb',
                       help='Prefix for feature names (e.g., emb_aud, emb_bip)')
    ...
    print(f"Feature prefix: {args.prefix}")
    print(f"Compute deltas: {config['embeddings']['pooling'].get('compute_delta', False)}")
    print(f"Hashtags: {len(hashtags_to_process)} to process\n")
    
    for i, hashtag in enumerate(tqdm(hashtags_to_process, desc="Hashtags")):
        try:
            features = process_hashtag(hashtag, config, 
